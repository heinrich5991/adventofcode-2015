...
print(max(itertools.permutations(cities), key=lambda path: sum(distances[(x, y)] for x, y in pairs(path))))
print(max(sum(distances[(x, y)] for x, y in pairs(path)) for path in itertools.permutations(cities)))
